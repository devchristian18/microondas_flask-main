...
# Datos de usuarios (en producción deberías usar una base de datos)
# y la contraseña debería estar hasheada
usuarios = {
    "admin": "admin123",
    "usuario": "password"
}
...
@app.route('/')
def home():
    # Redirigir a la página principal si está logueado o al login si no
...
def index():
    with open('recetas.json', 'r') as f:
        recetas = json.load(f)
    return render_template('index.html', recetas=recetas, usuario=session.get('usuario'))
...
def dashboard():
    ...
    return render_template('dashboard.html', programaciones=programaciones_formateadas, usuario=session.get('usuario'))
...
def personalizar_receta(receta_id):
    with open('recetas.json', 'r') as f:
        ...
        if receta_a_personalizar:
            return render_template('personalizar.html', receta=receta_a_personalizar, usuario=session.get('usuario'))
        else:
